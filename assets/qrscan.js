// assets/qrscan.js
export const ScanState = { running:false, stream:null, track:null, detector:null, torchOn:false, lastValue:'' };
...
export async function startScan(opts = {}) {
  ...
  if ('BarcodeDetector' in window) {
    try { ScanState.detector = new BarcodeDetector({ formats: ['qr_code'] }); }
    catch (e) { ScanState.detector = null; }
  }
...
export async function toggleTorch() {
  if (!ScanState.track?.getCapabilities) return false;
  const cap = ScanState.track.getCapabilities();
